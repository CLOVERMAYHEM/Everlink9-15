...
const ALLOWED_INVITE_LINKS = [
  'KSWesbA2Yz'  // Just the invite code
];
...
async function handleInviteLinkDetection(message) {
  ...
  if (matches) {
    ...
    const hasUnallowedInvite = matches.some(invite => {
      ...
      // Check if this invite code is in the whitelist
      return !ALLOWED_INVITE_LINKS.includes(inviteCode);
    });
...
if (!token) {
  console.error("❌ No Discord token found! Please set DISCORD_BOT_TOKEN environment variable.");
  process.exit(1);
}
